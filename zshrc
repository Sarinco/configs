...
# Adding a little function to mkdir and cd at the same time
mkcd () {
	mkdir "$1"
	cd "$1"
}
...
__conda_setup="$('/home/antoined/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/antoined/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/home/antoined/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/antoined/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
#contiki setup
export CNG_PATH=~/Applications/contiki-ng
